...
-removed([{set_attribute,3,"use emilio_erl_anno:set_line/2 instead"},
          {attributes_info,'_',
           "emilio_erl_anno:{column,line,location,text}/1 instead"},
          {token_info,'_',
           "erl_scan:{category,column,line,location,symbol,text}/1 instead"}]).

-removed_type([{column,0,"use emilio_erl_anno:column() instead"},
               {line,0,"use emilio_erl_anno:line() instead"},
               {location,0,"use emilio_erl_anno:location() instead"}]).
...
-type token() :: {category(), Anno :: emilio_erl_anno:anno(), symbol()}
               | {category(), Anno :: emilio_erl_anno:anno()}.
-type tokens() :: [token()].
-type error_description() :: term().
-type error_info() :: {emilio_erl_anno:location(), module(), error_description()}.
...
-spec string(String) -> Return when
      ...
      EndLocation :: emilio_erl_anno:location(),
      ErrorLocation :: emilio_erl_anno:location().
string(String) ->
    string(String, 1, []).
...
-spec string(String, StartLocation) -> Return when
      ...
      Return :: {'ok', Tokens :: tokens(), EndLocation}
              | {'error', ErrorInfo :: error_info(), ErrorLocation},
      StartLocation :: emilio_erl_anno:location(),
      EndLocation :: emilio_erl_anno:location(),
      ErrorLocation :: emilio_erl_anno:location().
...
-type cont_fun() :: fun((char_spec(), #erl_scan{},
                         emilio_erl_anno:line(), emilio_erl_anno:column(),
                         tokens(), any()) -> any()).
-opaque return_cont() :: {erl_scan_continuation,
                          string(), emilio_erl_anno:column(), tokens(),
                          emilio_erl_anno:line(),
                          #erl_scan{}, any(), cont_fun()}.
-type tokens_result() :: {'ok', Tokens :: tokens(),
                          EndLocation :: emilio_erl_anno:location()}
                       | {'eof', EndLocation :: emilio_erl_anno:location()}
...
-spec column(Token) -> emilio_erl_anno:column() | 'undefined' when
      Token :: token().

column(Token) ->
    emilio_erl_anno:column(element(2, Token)).

-spec end_location(Token) -> emilio_erl_anno:location() | 'undefined' when
      Token :: token().

end_location(Token) ->
    emilio_erl_anno:end_location(element(2, Token)).

-spec line(Token) -> emilio_erl_anno:line() when
      Token :: token().

line(Token) ->
    emilio_erl_anno:line(element(2, Token)).

-spec location(Token) -> emilio_erl_anno:location() when
      Token :: token().

location(Token) ->
    emilio_erl_anno:location(element(2, Token)).

-spec text(Token) -> emilio_erl_anno:text() | 'undefined' when
      Token :: token().

text(Token) ->
    emilio_erl_anno:text(element(2, Token)).
...
anno(Line, no_col, #erl_scan{text = true}, String) ->
    Anno = anno(Line),
    emilio_erl_anno:set_text(String, Anno);
