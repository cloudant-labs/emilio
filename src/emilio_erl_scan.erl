...
scan1([C|Cs], St, Line, Col, Toks) when C >= 223, C =< 255, C =/= 247 ->
    scan_atom(Cs, St, Line, Col, Toks, [C]);
scan1([C|Cs], St, Line, Col, Toks) when C >= 192, C =< 222, C /= 215 ->
    scan_variable(Cs, St, Line, Col, Toks, [C]);
...
scan_name([$@=C|Cs], Ncs) ->
    scan_name(Cs, [C|Ncs]);
scan_name([C|Cs], Ncs) when C >= 223, C =< 255, C =/= 247 ->
    scan_name(Cs, [C|Ncs]);
scan_name([C|Cs], Ncs) when C >= 192, C =< 222, C /= 215 ->
    scan_name(Cs, [C|Ncs]);
scan_name([], Ncs) ->
    {more,Ncs};
scan_name(Cs, Ncs) ->
