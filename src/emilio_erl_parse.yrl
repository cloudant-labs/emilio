...
%% Removed functions
-removed([{set_line,2,"use emilio_erl_anno:set_line/2"},
		  {get_attributes,1,"emilio_erl_anno:{column,line,location,text}/1 instead"},
		  {get_attribute,2,"emilio_erl_anno:{column,line,location,text}/1 instead"}]).

%% Start of Abstract Format

-type anno() :: emilio_erl_anno:anno().
...
-type form_info() :: {'eof', emilio_erl_anno:line()}
...
-type error_info() :: {emilio_erl_anno:line(), module(), error_description()}.
...
-spec error_bad_decl(emilio_erl_anno:anno(), attributes()) -> no_return().
...
-spec abstract(Data, Options) -> AbsTerm when
	  ...
	  Line :: emilio_erl_anno:line(),
	  AbsTerm :: abstract_expr().

abstract(T, Line) when is_integer(Line) ->
	Anno = emilio_erl_anno:new(Line),
	abstract(T, Anno, enc_func(epp:default_encoding()));
...
-spec map_anno(Fun, Abstr) -> NewAbstr when
	  ...
	  Anno :: emilio_erl_anno:anno(),
	  NewAnno :: emilio_erl_anno:anno(),
...
new_anno(Term) ->
	F = fun(L, Acc) -> {emilio_erl_anno:new(L), Acc} end,
...
anno_to_term(Abstract) ->
	F = fun(Anno, Acc) -> {emilio_erl_anno:to_term(Anno), Acc} end,
...
anno_from_term(Term) ->
	F = fun(T, Acc) -> {emilio_erl_anno:from_term(T), Acc} end,
